...
-- Arrow keys also switch between windows
...
require('lazy').setup({
  ...
  },
  {
    ...
    config = function()
      ...
      local servers = {
        ...
        -- Deno — only active inside supabase/functions
        denols = {
          root_dir = require('lspconfig.util').root_pattern 'supabase/functions',
          settings = {
            deno = {
              enable = true,
              suggest = { imports = { hosts = { ['https://esm.sh'] = true } } },
              unstable = true,
            },
          },
        },
      ...
      require('mason-tool-installer').setup {
        ensure_installed = {
          ...
          'djlint', -- Django/Jinja template formatter + linter
          'debugpy', -- Python/Django debugger adapter
        },
      }
  ...
  { -- Kanagawa palette aligned with the opencode theme.
    ...
    config = function()
      ...
      -- Small highlight tweaks so tmux/alacritty/opencode feel like one palette.
      vim.api.nvim_set_hl(0, 'Comment', { italic = false })
      vim.api.nvim_set_hl(0, 'FloatBorder', { fg = '#2D4F67', bg = 'NONE' })
